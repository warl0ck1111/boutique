...
public class PropertyController {
    ...
    @PostMapping()
    public ResponseEntity<ApiResponse> CreateProperty(@RequestBody PropertyRequestDto propertyDto){
        return ResponseEntity.ok(new ApiResponse("Property created successfully",propertyService.save(propertyDto)));
    }

    @PutMapping("{PropertyId}")
    public ResponseEntity<ApiResponse> updateProperty(@PathVariable Long PropertyId, @RequestBody PropertyRequestDto propertyDto){

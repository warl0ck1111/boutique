...
public interface ProductMediaRepository extends JpaRepository<ProductMedia,Long> {
    ...
    @Query(value = "SELECT p.created_at AS createdAt, p.description, p.modified_at AS modifiedAt, " +
            ...
            "FROM product_media pi LEFT JOIN product p on pi.product_id = p.id where pi.product_id = :productId", nativeQuery = true)
    List<ProductResponseDto> findAllByProductId(@Param("productId") Long productId);

    @Modifying
    @Query(value = "INSERT INTO product_media ( created_at, file_name,file_type, modified_at, product_id) VALUE(:createdAt, :fileName, :fileType, :modifiedAt, :productId)", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "DELETE FROM product_media where product_id = :productId", nativeQuery = true)
